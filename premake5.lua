...
function IsServerUsedDep(name)
    ...
    return name == "lua"
        ...
        or name == "sqlite"
        or name == "lzma"
        or SERVER_ZIP_SUPPORT and (name == "irrlicht" or name == "zlib")
end
...
local function ResolvePreBuiltDependencyDirectory(dep)
    if LUA_DEB and dep.name == "lua" then return end
...
if GetParam("lua-deb") then
    -- lua-deb is a shortcut for using the Debian lua package, which provides lua5.*-c++ and is compatible with ocgcore.
    LUA_DEB = true
    BUILD_LUA = false
    for _, version in ipairs({ "5.4", "5.3" }) do
        local lib_name = "lua" .. version .. "-c++"
        local lib_dir = os.findlib(lib_name)
        if lib_dir then
            print("Found lua " .. version .. " at " .. lib_dir)
            LUA_INCLUDE_DIR = path.join("/usr/include", "lua" .. version)
            LUA_LIB_NAME = lib_name
            LUA_LIB_DIR = lib_dir
            break
        end
    end
    if not LUA_LIB_DIR then
...
if SERVER_MODE then
        -- Keep the old name used by existing server deployments.
        startproject "ygopro"
else
        startproject "YGOPro"
end
    ...
    include "gframe"
    for _, dep in ipairs(DEPENDENCIES_METADATA) do
        if IsServerUsedDep(dep.name) and _G["BUILD_" .. string.upper(dep.name)] then
...
if SERVER_MODE and dep.name == "irrlicht" then
